    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <div>
                    <h2 className="text-3xl font-bold text-gray-900">Billing History</h2>
                    <p className="text-gray-500">View and manage all room bills for a specific period</p>
                </div>
            </div>

            <div className="flex gap-4 p-4 bg-white rounded-xl border border-gray-100 shadow-sm items-center">
                <Calendar className="w-5 h-5 text-indigo-600" />
                <span className="text-sm font-bold text-gray-400 uppercase tracking-widest mr-2">Select Period:</span>
                <select
                                    ...
                                    return (
                                        <tr key={bill.id} className="hover:bg-gray-50/50 transition-colors">
                                            ...
                                            <td className="px-6 py-4 text-sm text-gray-600">
                                                {bill.room.tenants.map((t: any) => t.name).join(', ') || <span className="italic text-gray-400">Empty</span>}
                                            </td>
                    ...
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden animate-in fade-in zoom-in duration-200 flex flex-col max-h-[90vh]">
                        ...
                        <div className="overflow-y-auto flex-1">
                            <div className="p-6 space-y-8">
                                {/* Component Breakdown */}
                                <div>
                                    <h4 className="text-sm font-bold text-gray-400 uppercase tracking-widest mb-4">Bill Breakdown</h4>
                                    <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
                                        {[
                                            ...
                                            <div key={item.label} className="p-3 bg-gray-50 rounded-xl border border-gray-100">
                                                ...
                                                <p className="text-lg font-black text-gray-900 mt-0.5">रु {(item.value || 0).toLocaleString()}</p>
                                            </div>
                                        ))}
                                        <div className="p-3 bg-indigo-50 rounded-xl border border-indigo-100">
                                            <p className="text-[10px] font-bold text-indigo-400 uppercase tracking-widest">Grand Total</p>
                                            <p className="text-lg font-black text-indigo-700 mt-0.5">रु {selectedBill.totalAmount.toLocaleString()}</p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                {/* Payment History */}
                                <div>
                                    <h4 className="text-sm font-bold text-gray-400 uppercase tracking-widest mb-4">Payment History</h4>
                                    {!selectedBill.paymentLogs || selectedBill.paymentLogs.length === 0 ? (
                                        <div className="p-8 text-center bg-gray-50 rounded-2xl border-2 border-dashed border-gray-200 text-gray-400 italic">
                                            No payments recorded for this bill.
                                        </div>
                                    ) : (
                                        <div className="space-y-3">
                                            {selectedBill.paymentLogs.map((log: any) => (
                                                <div key={log.id} className="p-4 bg-white border border-gray-100 rounded-xl shadow-sm flex justify-between items-center group hover:border-indigo-200 transition-colors">
                                                    <div className="flex items-center gap-4">
                                                        <div className={clsx(
                                                            "p-2.5 rounded-lg",
                                                            log.paymentType === 'WORK' ? "bg-amber-100 text-amber-600" : "bg-green-100 text-green-600"
                                                        )}>
                                                            {log.paymentType === 'WORK' ? <Briefcase className="w-5 h-5" /> : <IndianRupee className="w-5 h-5" />}
                                                        </div>
                                                        <div>
                                                            <div className="flex items-center gap-2">
                                                                <span className="font-black text-gray-900">रु {log.amount.toLocaleString()}</span>
                                                                {log.paymentType === 'WORK' && (
                                                                    <span className="text-[10px] font-bold bg-amber-50 text-amber-700 px-1.5 py-0.5 rounded border border-amber-100 uppercase">Work Log</span>
                                                                )}
                                                            </div>
                                                            <p className="text-xs text-gray-500 mt-0.5">{log.remarks || 'Standard Payment'}</p>
                                                        </div>
                                                    </div>
                        ...
                        <div className="p-6 border-t border-gray-100 bg-gray-50 shrink-0">
                            <div className="flex justify-between items-center mb-4">
                                <span className="text-sm font-bold text-gray-400 uppercase tracking-widest">Payment Status</span>
                                <div className="text-right">
                                    <p className="text-xs text-gray-400">Total Paid</p>
                                    <p className="font-black text-green-600">रु {selectedBill.paidAmount.toLocaleString()}</p>
                                </div>
                            </div>
                            <button
                                onClick={() => setIsDetailsModalOpen(false)}
                                className="w-full py-3 bg-gray-200 hover:bg-gray-300 text-gray-700 font-bold rounded-xl transition-colors uppercase tracking-widest text-sm"
                            >
                                Close Details
                            </button>
                        </div>
